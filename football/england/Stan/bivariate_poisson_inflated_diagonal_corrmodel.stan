...
parameters {
  ...
  vector[K] home_advantage_off;
  vector[K] home_advantage_def;
...
model {
  ...
  for (n in 1:N) {
    ...
    // Log-rate for the Poisson distribution modeling diagonal (tie) scores
    real log_lambda_tie = tie_alpha + (1 + tie_beta) * (mu[1] + mu[2]) / 2;

    goals1_2[ , n] ~ poisson_2d_log_inflated(mu[1], mu[2], mu3, p, log_lambda_tie);
...
generated quantities {
  ...
  for (n in 1:N_pred) {
    ...
    goals_pred[n] = poisson_2d_log_inflated_rng(mu[1], mu[2], mu3, p, log_lambda_tie);
