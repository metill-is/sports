...
#' Read a page of football data without the scores
#'
#' @param page A page of football data as read by rvest::read_html_live()
#' @return A tibble of the football data
#' @export
...
#' Make a results URL for English football league
#'
...
make_results_url <- function(league_name, year_start, year_end) {
  ...
  glue(
    "https://www.livesport.com/en/soccer/england/{league_name}-{year_start}-{year_end}/results/"
  )
}

#' Make a schedule URL for English football league
#'
#' @param league_name The name of the league
...
make_schedule_url <- function(league_name) {
  ...
  glue(
    "https://www.livesport.com/en/soccer/england/{league_name}/fixtures/"
  )
}
