...
generated quantities {
  ...
  array[N] int<lower=0> goals1_rep;
  array[N] int<lower=0> goals2_rep;
  ...
  for (n in 1:N) {
    ...
    off_rep[1] = offense[round1[n], team1[n]] + home_advantage_off[team1[n]];
    def_rep[1] = defense[round1[n], team1[n]] + home_advantage_def[team1[n]];
    off_rep[2] = offense[round2[n], team2[n]];
    def_rep[2] = defense[round2[n], team2[n]];

    mu_rep[1] = mean_log_goals + off_rep[1] - def_rep[2];
    mu_rep[2] = mean_log_goals + off_rep[2] - def_rep[1];

    real strength_diff_rep = abs(off_rep[1] + def_rep[1] - off_rep[2] - def_rep[2]);

    real logit_rho_rep = alpha_mu3 + beta_mu3_strength_diff * strength_diff_rep;
    real mu3_rep = log_inv_logit(logit_rho_rep) + 0.5 * (mu_rep[1] + mu_rep[2]);

    real logit_p_rep = logit_p0 + beta_logit_p_strength_diff * strength_diff_rep;
    real p_rep = inv_logit(logit_p_rep);
    real lambda_tie_rep = tie_alpha + (1 + tie_beta) * (mu_rep[1] + mu_rep[2]) / 2;

    array[2] int rep = poisson_2d_log_inflated_rng(mu_rep[1], mu_rep[2], mu3_rep, p_rep, lambda_tie_rep);
    goals1_rep[n] = rep[1];
    goals2_rep[n] = rep[2];
  }
