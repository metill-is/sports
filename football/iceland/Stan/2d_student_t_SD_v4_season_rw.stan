/**
 * Football (S, D) model — v4 = v3 + season-varying mean_goals.
 *
 * Same as 2d_student_t_SD_v3_free_nu.stan, but mean_goals evolves
 * across seasons via a random walk:
 *   mean_goals[s] = mean_goals[s-1] + delta_mean_goals + sigma_mean_goals * z_mean_goals[s-1]
 *
 * Hypothesis: multi-year scoring drift (Icelandic football has 8-13
 * seasons of data spanning 2018-2026) closes part of the 1.1% gap
 * that v3 has to BVP.
 *
 * Params added vs v3: delta_mean_goals, sigma_mean_goals, z_mean_goals
 * (length N_seasons - 1). For Iceland N_seasons ~ 13 so ~14 params total.
 * Cheap and well-behaved — this structure was in direct_SD already and
 * sampled fine (the problem there was the scale_sigma_def hierarchy,
 * not the season RW).
 *
 * Priors chosen to match direct_SD's originals so we can trace behaviour.
 */
...
parameters {
  ...
  // Season-varying mean_goals (random walk)
  real mean_goals0;
...
transformed parameters {
  ...
  // Season RW for mean_goals
  vector[N_seasons] mean_goals;
  mean_goals[1] = mean_goals0;
  for (s in 2:N_seasons) {
    mean_goals[s] = mean_goals[s-1] + delta_mean_goals
      + sigma_mean_goals * z_mean_goals[s-1];
  }
...
model {
...

  {
    ...
    for (n in 1:N) {
      ...
      mu_SD[n][1] = 2 * mu + (off_h + off_a) - (def_h + def_a);
