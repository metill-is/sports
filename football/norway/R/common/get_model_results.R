...
predictions <- posterior_goals |>
  ...
  mutate(
    ...
    match = str_c(home, " - ", away) |>
      fct_reorder(game_nr),
    division = c("ES", "OL", "ÖD", "ÞD", "NM")[division]
  ) |>
  mutate(
    home = glue(
...
plot <- predictions |>
  ...
  labs(
    ...
    colour = NULL,
    title = paste0(
      "Vikuleg fótboltaspá Metils fyrir Eliteserien (ES), OBOS-Ligaen (OL) og suma mótsleiki (NM)"
    ),
    subtitle = str_c(
...
plot_dat |>
  ...
  labs(
    title = "Hvaða lið munu komast í efri hluta umspils Eliteserien",
    subtitle = str_c(
...
plot_dat |>
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "Líkindadreifing yfir lokasæti í Eliteserien fyrir umspil haustsins"
  )
...
results$draws("home_advantage_tot") |>
  ...
  labs(
    ...
    y = NULL,
    colour = NULL,
    title = "Heimavallaráhrif í Eliteserien",
