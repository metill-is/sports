...
handball_bets <- read_sheet(Sys.getenv("ODDS_SHEET"), sheet = "Bets") |> 
  mutate(
...
bets |> 
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "Samanburður á væntum og raunverulegum gróða í handboltaveðmálum"
  )
...
bets |> 
  ...
  summarise(
    cumul_change = tail(cumul_change, 1),
    .by = c(dags_leikur, country, iter)
  ) |> 
  summarise(
    lower = quantile(cumul_change, 0.025),
    upper = quantile(cumul_change, 0.975),
    .by = c(dags_leikur, country)
  ) |> 
  inner_join(
    bets |> 
      filter(
