#!/usr/bin/env Rscript
# Handball Iceland male — refit both variants with adapt_delta = 0.95.
#
# Motivation: divergences (~1% on both tier1 and scalarsigma) cluster at
# LARGE scale_sigma_def values (ratio of div/ok mean ~1.9–2.0x). E-BFMI is
# 0.9–1.04 on every chain — no global funnel pathology. This is localised
# leapfrog stiffness in the tail, the textbook adapt_delta band-aid case.
#
# Outputs:
#   handball/iceland/results/male/fit_tier1_adapt95.rds
#   handball/iceland/results/male/fit_scalarsigma_adapt95.rds
#   handball/iceland/results/male/compare_2026_04_20_adapt95.txt
...
cat(sprintf(
  "  K=%d  N=%d  N_rounds=%d  N_seasons=%d\n",
  stan_data$K, stan_data$N, stan_data$N_rounds, stan_data$N_seasons
))

results_dir <- here("handball", "iceland", "results", "male")
out_tier1 <- file.path(results_dir, "fit_tier1_adapt95.rds")
out_scalar <- file.path(results_dir, "fit_scalarsigma_adapt95.rds")
log_path <- file.path(results_dir, "compare_2026_04_20_adapt95.txt")

fit_variant <- function(name, stan_path) {
  cat(sprintf(
    "\n--- %s: compiling + fitting %s with adapt_delta=0.95 ---\n",
    name, stan_path
  ))
...
res_tier1 <- fit_variant("tier1", "handball/iceland/Stan/2d_student_t.stan")
cat("\n--- Saving tier1 fit ---\n")
...
sink(log_path)
cat("# Handball Iceland male — adapt_delta=0.95 refit\n")
cat(sprintf(
  "# Fitted %s, 4 chains x (1000 warmup + 1000 sampling)\n",
