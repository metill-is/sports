/**
 * Football (S, D) model — v2 = barebones + per-team home advantage.
 *
 * Builds on 2d_gaussian_SD_v1_barebones.stan by re-introducing K-dim
 * home advantage (home effects can legitimately differ between teams —
 * different home grounds, fans, altitude/turf, etc.). Everything else
 * stays barebones:
 *   - Scalar sigma_off, sigma_def (no per-team innovation variance)
 *   - Scalar mean_goals0
 *   - Gaussian likelihood (no nu)
 *
 * Hypothesis: per-team home advantage closes part of the 1.6% log-score
 * gap to BVP without re-introducing the scale_sigma_def funnel that
 * broke direct_SD.
 *
 * Params added vs v1: 2*(K-1) ~ 134 per-team home advantage, but they
 * are weakly coupled (no hierarchy over teams here — each team has its
 * own prior N(0.15, 0.3) independently). Should fit cleanly.
 */
