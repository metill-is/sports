...
process_leagues_historical_data <- function(
  ...
  league_list <- list(
    "eliteserien" = 1,
    "obos-ligaen" = 2,
    "division-2-group-1" = 3,
    "division-2-group-2" = 3,
    "division-3-group-1" = 5,
    "division-3-group-2" = 5,
    "division-3-group-3" = 5,
    "division-3-group-4" = 5,
    "division-3-group-5" = 5,
    "division-3-group-6" = 5,
    "nm-cup" = 6
  )
...
process_schedule <- function() {
  ...
  for (sex in sexes) {
    # Which leagues are available?
    leagues <- list.files(sex, full.names = TRUE, pattern = "eliteserien|-")
    ...
    data <- map(
      ...
      mutate(
        date = clock::date_build(2025, month, day)
      ) |>
