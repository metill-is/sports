...
parameters {
  ...
  // goals per sqrt-day (small round-to-round drift); scale_sigma_off ~ exponential(2)
...
transformed parameters {
  ...
  // lognormal hierarchy. Typical magnitude ~exp(-1.5) ~ 0.22 goals per sqrt-day.
  ...
  // sigma_def[k]: per-team defensive RW SD (mean_sigma_def -> exp() ~ 0.14 goals/sqrt-day).
