...
sheet_url <- "https://docs.google.com/spreadsheets/d/1ZFa79A3fEXcAkxDz6cxbCMby4Q2WUT0CI9dp6N4UQlk/edit?gid=1774709455#gid=1774709455"
...
outcome_odds <- read_sheet(
  ...
  filter(
    ...
    .by = c(date_game, booker, home, away)
  ) |>
...
outcome_results <- post |>
  ...
  summarise(
    ...
    .by = c(date, division, home, away, sex)
  ) |>
  inner_join(
    outcome_odds |>
      select(
  ...
  select(date, division, booker, sex, home, away, everything()) |>
  pivot_longer(
    -c(date, division, booker, sex, home, away),
  ...
  mutate(
    kelly = get_kelly(p, o),
    .by = c(gestir, division, date, sex, heima, booker)
  ) |>
  filter(
    kelly == max(kelly),
    .by = c(heima, division, date, sex, gestir, outcome)
  ) |>
  group_by(date, division, heima, sex, gestir, outcome) |>
  slice(1) |>
  ungroup() |>
  mutate(
    kelly = get_kelly(p, o),
    .by = c(date, division, sex, heima, gestir)
  ) |>
  mutate(
...
forgjof_results <- handicap_odds |>
  ...
  mutate(
    kelly = get_kelly(p, o),
    .by = c(booker, sex, gestir, heima, change)
  ) |>
  filter(
    kelly == max(kelly),
...
stigafjoldi_results <- post |>
  ...
  summarise(
    ...
    p_under = 1 - p_over,
    .by = c(date, division, sex, heima, gestir, booker, o_over, o_under, limit)
  ) |>
  pivot_longer(
  ...
  mutate(
    kelly = get_kelly(p, o),
    .by = c(booker, gestir, sex, heima, limit)
  ) |>
  filter(
    kelly == max(kelly),
    .by = c(heima, gestir, outcome, sex)
  ) |>
  group_by(sex, date, heima, gestir, outcome) |>
  slice(1) |>
  ungroup() |>
  mutate(
    kelly = get_kelly(p, o),
...
post |>
  ...
  select(date, division, sex, booker, home, away, everything()) |>
  pivot_longer(
    -c(date, division, booker, sex, home, away),
    names_to = c("type", "outcome"),
    names_sep = "_"
  ...
  mutate(
    kelly = get_kelly(p, o),
    .by = c(gestir, sex, division, date, heima, booker)
  ) |>
  filter(
    kelly == max(kelly),
    .by = c(heima, sex, division, date, gestir, outcome)
  ) |>
  group_by(date, division, sex, heima, gestir, outcome) |>
  slice(1) |>
  ungroup() |>
  mutate(
    kelly = get_kelly(p, o),
  ...
  anti_join(
    bets,
    by = join_by(heima, gestir, type == tegund)
  ) |>
  remove_colnames() |>
...
handicap_odds |>
  ...
  mutate(
    kelly = get_kelly(p, o),
    .by = c(booker, gestir, heima, change, sex)
  ) |>
  filter(
    kelly == max(kelly),
    .by = c(heima, gestir, outcome, sex)
...
post |>
  ...
  filter(
    kelly == max(kelly),
    .by = c(heima, sex, gestir, outcome)
  ) |>
  group_by(date, sex, heima, gestir, outcome) |>
  slice(1) |>
  ungroup() |>
  mutate(
    kelly = get_kelly(p, o),
