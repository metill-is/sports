...
#'
#' DEPRECATED (2026-04-22): Violates the workspace invariant that only
#' `lengjan-bets/` writes to `bets_log.csv`. Retained as a manual-override
#' escape hatch for cases where a bet was placed outside the normal flow
#' (e.g. manually on Lengjan's site) and needs to be back-logged. For the
#' standard flow, use `/place-bets` or `Rscript lengjan-bets/run.R`.
#' Duplicates logic in `lengjan-bets/R/pipeline.R::log_placed_bet()`.
#'
#' Usage:
...
if (nrow(recs) == 0) {
  cat("No recommendations to show.\n")
  quit(status = 0)
}
...
table_header <- function() {
  cat(sprintf(
    "%-4s %-12s %-7s %-20s %-10s %5s %6s %6s %7s %7s\n",
...
table_row <- function(r) {
  ...
  cat(sprintf(
    "%-4d %-12s %-7s %-20s %-10s %5.2f %5.1f%% %5.1f%% %6.0f %+6.0f\n",
...
if (length(dates) > 1) {
  cat(sprintf(
    "=== ALL DAYS: %d bets, %s kr stake, %+.0f kr expected ===\n\n",
...
for (i in seq_len(nrow(placed))) {
  ...
  info <- if (r$market == "totals" && !is.na(r$limit)) {
    as.character(r$limit)
  } else if (r$market == "handicap" && !is.na(r$change)) {
    as.character(r$change)
  } else {
    NA_character_
  }
  ...
  cat(
    "  Logged:", r$heima, "v", r$gestir, "(", r$market, r$outcome, ")",
    r$bet_amount, "kr @", r$o, "\n"
  )
}
