#' @include placer-ledger.R
NULL

#' Create an authenticated Lengjan browser session
#'
#' Establishes a Chromote session, navigates to games.lotto.is, fills
#' credentials from the `LENGJAN_USER` / `LENGJAN_PASS` environment variables,
#' and returns the live session ready for navigation.
#'
#' Credentials are gated: missing or empty `LENGJAN_USER` / `LENGJAN_PASS`
#' produce a clean error before Chrome is opened.
#'
#' @param headless Run headless? Default `TRUE` for CI; pass `FALSE` to watch
#'   the browser click through.
#' @param timeout_s Seconds to wait after navigating / submitting (default 30;
#'   currently used as per-step wait budget).
#' @return A `ChromoteSession` object, logged in to Lengjan.
#' @export
chromote_login <- function(headless = TRUE, timeout_s = 30) {
  ...
  if (!isTRUE(result$result$value)) {
    stop("Could not find 'M\u00ednar s\u00ed\u00f0ur' button.", call. = FALSE)
  }
  Sys.sleep(2)
  ...
  if (!isTRUE(fill_result$result$value)) {
    stop("Could not find username/password inputs in login modal.", call. = FALSE)
  }
  Sys.sleep(0.5)
...
#' Check if a Chromote session is still authenticated to Lengjan
#'
#' @param session A `ChromoteSession` object.
#' @return Logical `TRUE` if the page shows a balance (balance indicator
#'   contains "kr."), `FALSE` otherwise or if the session is dead.
#' @export
