#!/usr/bin/env Rscript
#
# Fit + score v5 (per-team sigma on (Y_h, Y_a)) and compare to v3 + BVP.
#
# Gates (per brief):
...
cat("==============================================\n")
cat(" v5 (per-team sigma, (Y_h, Y_a) transform) audit\n")
cat("==============================================\n")
...
stan_path <- file.path(
  ...
  "2d_student_t_SD_v5_perteam_sigma.stan"
)
audit_dir <- file.path(
  sports_dir, "football", "iceland", "results", "male",
  "audits", "student_t"
)
fit_path <- file.path(audit_dir, "fit_student_t_v5_perteam_sigma.rds")

res <- fit_variant(
  stan_path = stan_path,
  stan_data = stan_data,
  label = "v5_perteam_sigma",
  seed = 20260420,
  iter_warmup = 1000,
  iter_sampling = 1000,
  chains = 4,
...
# v5 emits per-match mu_D_fit, sigma_D_fit directly — bypass the
# scalar-sigma_D assumption in direct_SD_outcome_probs.
v5_outcome_probs <- function(fit, stan_data,
...
cat(sprintf("\n[v5] analytical Student-t 1x2 (per-match sigma_D)...\n"))
t0 <- Sys.time()
probs_v5 <- v5_outcome_probs(res$fit, stan_data, threshold = 0.5, n_draws = 200)
cat(sprintf("  wall: %.1f s\n", as.numeric(difftime(Sys.time(), t0, units = "secs"))))
s_v5_1x2 <- score_outcome(probs_v5, obs_g1, obs_g2, "v5_perteam_sigma")

# ---- Totals calibration (reuse direct_SD_totals_probs via total_goals_rep) ---
cat(sprintf("\n[v5] totals from total_goals_rep...\n"))
t0 <- Sys.time()
p_v5 <- direct_SD_totals_probs(res$fit, stan_data,
  lines = c(0.5, 1.5, 2.5, 3.5, 4.5, 5.5), n_draws = 200
)
cat(sprintf("  wall: %.1f s\n", as.numeric(difftime(Sys.time(), t0, units = "secs"))))
s_v5_totals <- score_totals(p_v5, obs_g1, obs_g2, "v5_perteam_sigma")

# ---- Comparison rows (from existing artefacts, don't refit) --------------
# Pull v3 and BVP baselines from Phase 4 artefacts.
...
# ---- Report --------------------------------------------------------------
out_path <- file.path(audit_dir, "compare_v5.txt")
con <- file(out_path, open = "wt")
on.exit(close(con), add = TRUE)
w <- function(...) cat(sprintf(...), file = con)

w("v5 (per-team sigma, (Y_h, Y_a)) — Iceland male\n")
w("Date: %s\n", format(Sys.Date()))
w(
  "Commit: %s\n",
...
w("--- Diagnostics ---\n")
print_diagnostics(d, "v5_perteam_sigma", con = con)
w("\n")

summ <- res$fit$summary(
  variables = c("mean_goals0", "sigma_off", "sigma_def", "rho_Y", "nu")
)
w("--- Posterior summaries (key scalars) ---\n")
capture.output(print(summ, digits = 3), file = con, append = TRUE)

sigma_team_summ <- res$fit$summary(variables = "sigma_team")
w("\n--- sigma_team (per-team observation scale) ---\n")
w("  n teams:       %d\n", nrow(sigma_team_summ))
w(
  "  min mean:      %.3f  (team %d)\n",
  min(sigma_team_summ$mean),
  which.min(sigma_team_summ$mean)
)
w("  median mean:   %.3f\n", median(sigma_team_summ$mean))
w(
  "  max mean:      %.3f  (team %d)\n",
  max(sigma_team_summ$mean),
  which.max(sigma_team_summ$mean)
)
w("  sd of means:   %.3f\n", sd(sigma_team_summ$mean))

w("\n--- 1x2 log-score comparison ---\n")
cmp_1x2 <- data.frame(
  model = c("v5_perteam_sigma", "v3_free_nu (baseline)", "BVP_no_inflation (target)"),
  log_score = c(s_v5_1x2$log_score, v3_1x2_log_score, bvp_1x2_log_score),
  rel_v3 = c(s_v5_1x2$log_score - v3_1x2_log_score, 0, bvp_1x2_log_score - v3_1x2_log_score),
  rel_bvp = c(s_v5_1x2$log_score - bvp_1x2_log_score, v3_1x2_log_score - bvp_1x2_log_score, 0)
)
capture.output(print(cmp_1x2, row.names = FALSE, digits = 5),
  file = con, append = TRUE
)

w("\n--- 1x2 detailed scores (v5) ---\n")
print_outcome_scores(s_v5_1x2, con)

w("\n--- Totals log-score comparison ---\n")
cmp_tot <- data.frame(
  model = c("v5_perteam_sigma", "v3_free_nu (baseline)", "BVP_no_inflation (target)"),
  mean_log = c(
    attr(s_v5_totals, "mean_log_score"),
    v3_totals_mean_log,
    bvp_totals_mean_log
  ),
  rel_v3 = c(
    attr(s_v5_totals, "mean_log_score") - v3_totals_mean_log,
    0,
    bvp_totals_mean_log - v3_totals_mean_log
  ),
  rel_bvp = c(
    attr(s_v5_totals, "mean_log_score") - bvp_totals_mean_log,
...
w("\n--- Totals per-line (v5) ---\n")
print_totals_scores(s_v5_totals, label = "v5_perteam_sigma", con = con)
...
improvement_1x2 <- s_v5_1x2$log_score - v3_1x2_log_score
improvement_totals <- attr(s_v5_totals, "mean_log_score") - v3_totals_mean_log
w(
...
cat(sprintf(
  ...
  s_v5_1x2$log_score, v3_1x2_log_score, bvp_1x2_log_score
))
cat(sprintf(
  "  Totals mean log:      %.4f (v3 %.4f, BVP %.4f)\n",
  attr(s_v5_totals, "mean_log_score"), v3_totals_mean_log, bvp_totals_mean_log
))
cat(sprintf("\n[report] Wrote %s\n", out_path))
