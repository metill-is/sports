...
box::use(
  ...
  dplyr[
    select, filter, arrange, mutate, tibble, row_number, rename,
...
prepare_football_data <- function(sex, from_season = 2021, test_date_cutoff = NULL) {
  ...
  if (!is.null(test_data)) {
    ...
    # Support Icelandic column names in schedule too
    if ("dags" %in% names(next_games)) {
      next_games <- next_games |> rename(
        date = dags, home = heima, away = gestir
      )
    }
