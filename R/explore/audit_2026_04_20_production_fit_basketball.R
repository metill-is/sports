#!/usr/bin/env Rscript
# Production-length fit of tier1 baseline vs scalarsigma variant.
#
# Two 4-chain × (1000 warmup + 1000 sampling) fits on basketball male Iceland,
# with log_lik in generated quantities so we can compute loo::loo elpd.
#
# Outputs:
#   results/male/fit_tier1.rds          — tier1 fit (audit-branch baseline)
#   results/male/fit_scalarsigma.rds    — scalarsigma fit (candidate)
#   results/male/compare_2026_04_20.txt — diagnostics + elpd comparison
#
# Gate for swapping leagues.yml: Rhat < 1.01, min ESS_bulk > 400,
# elpd_diff within ~1 SE (prefer scalarsigma by Occam's razor).
...
# -------- Paths ---------------------------------------------------------
results_dir <- here("basketball", "iceland", "results", "male")
...
fit_variant <- function(name, stan_path) {
  ...
  fit <- mod$sample(
    ...
    refresh = 100,
    seed = 20260420,
    init = 0, # load-bearing: random init makes rho near ±1
    show_messages = TRUE,
...
res_tier1 <- fit_variant(
  "tier1",
  "basketball/iceland/Stan/2d_student_t.stan"
)
cat("\n--- Saving tier1 fit ---\n")
...
res_scalar <- fit_variant(
  "scalarsig",
  "basketball/iceland/Stan/2d_student_t_scalarsigma.stan"
)
...
# -------- Write log file -------------------------------------------------
sink(log_path)
cat("# Basketball Iceland male — production-length fit comparison\n")
...
cat("\n### scalarsigma\n")
print(loo_scalar)
cat("\n### loo_compare (negative elpd_diff => first row is best)\n")
print(loo_comp)
cat("\n## Key parameters\n\n")
for (row in summary_rows) {
  cat(sprintf(
