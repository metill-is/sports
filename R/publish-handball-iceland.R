...
#' Publish handball Iceland JSONs (Plan 4 scaffold).
#'
#' Produces a minimal pair of JSONs (`meta.json` + `next_games.json`) for
#' handball Iceland into `output_root/handball/iceland/{karla,kvenna}/`.
#' League-table / season-projection / team-strengths JSONs are deferred to
#' Plan 6 -- they need the metill-platform handball page to be designed
#' first.
#'
...
publish_handball_iceland <- function(fit, league, sex,
  ...
  # points_distribution.json -- deferred until metill-platform handball
  # page is designed.

  message(sprintf(
    "publish_handball_iceland: wrote 2 JSONs to %s", out_dir
  ))
  invisible(NULL)
}
