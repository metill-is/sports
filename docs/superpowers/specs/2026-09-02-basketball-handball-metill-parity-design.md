...
| N8 | **The F6 extract path is NOT bit-rotted** | Ran `extract_basketball_iceland()` against the real stored fit (`data/beliefs/fits/sport=basketball/.../fit.rds`, 598 MB, class `CmdStanMCMC`) on 2026-09-02. Result OK; wrote exactly the 5 expected parquets. The spec's "has it rotted after three months of football-driven refactors" risk is **closed**, and the separate verification workstream it implied is unnecessary. |
| N9 | **B5 quantified against real data** | Same run. Raw Stan `home_advantage_tot` medians across teams: min **1.50**, median **4.32**, max **12.07** points. Published via `exp(x/2)`: a 1.5-point edge ships as 2.12 (+41%), a 4.32-point edge as **8.67** (2x), a 12.07-point edge as **420** (35x). The low end is plausible enough to pass review while the top of the same column is absurd -- which is why it would have shipped. |
| N10 | **`n_pred` mismatch is structural to the fit-RDS path** | Same run warned `fit was trained with N_pred=8 but prepare_data returned 2. Posterior-dependent JSONs will be empty` (`R/publish-iceland-2dt-helpers.R:84-86`), and `predicted_matches` came back empty. Cause: the fit-RDS path calls `prepare_data()` **at publish time**, re-deriving upcoming fixtures at a different date than the fit used. The extracts path reads pre-computed parquets and is structurally immune. **Another argument for WS9, not merely a bug.** |
