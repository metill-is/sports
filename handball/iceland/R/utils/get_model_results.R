...
generate_model_results <- function(sex = "male", end_date = Sys.Date()) {
  ...
  plot_dat |>
    ...
    labs(
      ...
      colour = NULL,
      title = glue(
        "Handboltaspá Metils fyrir Olís og Grill 66 deildir {translate_sex(sex)}"
      ),
      subtitle = str_c(
  ...
  p_top |>
    ...
    tab_header(
      title = glue("Hvernig endar Olís deild {translate_sex(sex)}?"),
      subtitle = "Handboltalíkan Metils fengið til að spá fyrir um niðurstöðu allra leikja á tímabilinu"
    ) |>
    tab_options(
  ...
  base_points <- d |>
    ...
    mutate(
      result = case_when(
        abs(home_goals - away_goals) < 0.5 ~ "tie",
        home_goals > away_goals ~ "home",
  ...
  plot_dat |>
    ...
    labs(
      title = glue(
        "Spá um stigafjölda í lok Olís deildar {translate_sex(sex)}"
      ),
      subtitle = str_c(
  ...
  plot_dat |>
    ...
    tab_header(
      title = glue(
        "Greining á sóknar-, varnar- og heildarstyrk í Olís deild {translate_sex(sex)}"
      ),
      subtitle = str_c(
  ...
  plot_dat |>
    ...
    labs(
      ...
      colour = NULL,
      title = glue(
        "Styrkur félagsliða í Olís deild {translate_sex(sex)}"
      ),
