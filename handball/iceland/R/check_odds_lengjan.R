...
odds_raw <- c("female", "male") |>
  map(
    \(sex) {
      path_raw <- here("odds", "iceland", sex) |>
...
odds <- odds_raw |>
  inner_join(
    names_table,
    by = join_by(home == lengjan_name, sex)
  ) |>
  select(-home) |>
  rename(home = data_name) |>
  inner_join(
    names_table,
    by = join_by(away == lengjan_name, sex)
  ) |>
  select(-away) |>
...
post <- c("female", "male") |>
  map(
    \(x) {
      here("results", x, Sys.Date(), "posterior_goals.csv") |>
        read_csv() |>
        mutate(
          sex = x
        )
