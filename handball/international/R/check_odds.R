...
hosts <- c("Norway", "Sweden", "Denmark")

sheet_url <- "https://docs.google.com/spreadsheets/d/1cE1LB2iuTwiV5GSf8JUSF3dO61Jhal0B3x0WrFpRADQ/edit?gid=1774709455#gid=1774709455"
...
outcome_odds <- outcome_odds |> 
  ...
  select(
    booker, date_obs, date_game, home, away, o_home, o_tie, o_away
  )
...
total_goals_odds <- total_goals_odds |> 
  ...
  select(
    booker, date_obs, date_game, home, away, limit, o_over, o_under
  )
...
handicap_odds <- handicap_odds |> 
  ...
  select(
    booker, date_obs, date_game, home, away, change, o_home, o_away
  )
