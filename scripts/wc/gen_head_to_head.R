...
# ---- validation: Portugal vs Argentina vs the standalone prototype ----
tv <- payload$teams
...
p_por_wins <- if (por_is_i) pr$p_i_wins_if_meet else 1 - pr$p_i_wins_if_meet
cat("\n== Portugal vs Argentina (validate vs standalone: .266 / .131 / .355) ==\n")
cat(sprintf("  P(Portugal farther)  = %.4f   (standalone 0.2657)\n", p_por_farther))
cat(sprintf("  P(tie)               = %.4f   (standalone 0.1123)\n", pr$p_tie))
cat(sprintf("  P(they meet)         = %.4f   (standalone 0.1307)\n", pr$p_meet))
cat(sprintf("  P(Portugal wins|meet)= %.4f   (standalone 0.3555)\n", p_por_wins))
cat(sprintf(
